...
K32_sampler::K32_sampler(const int SD_PIN[4]) {
...
String K32_sampler::path(int note, int bank) {
...
void K32_sampler::bank(int bank) {
  xSemaphoreTake(this->lock, portMAX_DELAY);
  this->_bank = bank;
  xSemaphoreGive(this->lock);
}

int K32_sampler::bank() {
  int b = 0;
  xSemaphoreTake(this->lock, portMAX_DELAY);
  b = this->_bank;
  xSemaphoreGive(this->lock);
...
int K32_sampler::size(byte note, byte bank) {
  int sizeN = 0;
...
void K32_sampler::remove(byte note, byte bank) {
  String path = this->path(note, bank);
  if (SD.exists(path)) {
    SD.remove(path);
    xSemaphoreTake(this->lock, portMAX_DELAY);
...
String K32_sampler::pad3(int input) {
  char bank[4];
  bank[3] = 0;
  bank[0] = '0' + input / 100;
...
void K32_sampler::task( void * parameter ) {
  K32_sampler* that = (K32_sampler*) parameter;
