...
class K32_light_anim_test : public K32_light_anim {
  public:
    K32_light_anim_test() {
      ...
      this->params[1] = 1000;   // initial delay
      this->params[2] = 250;    // step duration
    }
    ...
    bool loop ( K32_ledstrip* strip ){
      ...
      this->lock();
      strip->all( pixelFromRGB(this->params[0], 0, 0) );
      strip->show();
      this->unlock();

      vTaskDelay(pdMS_TO_TICKS(wait));
      this->lock();
      strip->all( pixelFromRGB(0, this->params[0], 0) );
      strip->show();
      this->unlock();
      ...
      this->lock();
      strip->all( pixelFromRGB(0, 0, this->params[0]) );
      strip->show();
      this->unlock();
      ...
      this->lock();
      strip->all( pixelFromRGBW(0, 0, 0, this->params[0]) );
      strip->show();
      this->unlock();
