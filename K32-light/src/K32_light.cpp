...
{
  for (int s = 0; s < this->_nfixtures; s++)
    this->_fixtures[s]->pix(pixel, color);
  return this;
}
...
void K32_light::command(Orderz *order)
{
  ...
  {
    int offset = 0;
    if (strcmp(order->action, "strip") == 0)
      offset = 1;
    if (strcmp(order->action, "pixel") == 0)
      offset = 2;

    if (order->count() < offset + 1)
      return;
  ...
  else if (strcmp(order->action, "master") == 0)
  {
    ...
    if (strcmp(order->subaction, "less") == 0)
      masterValue -= 2;
    else if (strcmp(order->subaction, "more") == 0)
      masterValue += 2;
    else if (strcmp(order->subaction, "full") == 0)
      masterValue = 255;
    else if (strcmp(order->subaction, "tenmore") == 0)
      masterValue += 10;
    else if (strcmp(order->subaction, "tenless") == 0)
      masterValue -= 10;
    ...
    else if (strcmp(order->subaction, "fadein") == 0)
    {
      ...
      masterValue = order->getData(0)->toInt();
  ...
  {
    LOG("DISPATCH: leds/maree");

    if (strcmp(order->subaction, "go_p") == 0)
    {
      int temps = (order->getData(0)->toInt() * 1000);
      int haute = order->getData(1)->toInt();
      int basse = order->getData(2)->toInt();
      int pixel = order->getData(3)->toInt();
      LOGF(" temps %d", temps);
      LOGF(" haute %d", haute);
      LOGF(" basse %d", basse);
      LOGF(" pixel %d", pixel);
      ...
      if (!this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod(new K32_mod_fadein)->name("fadein")->at(0)->at(7)->period(temps)->mini(basse)->maxi(haute);
        this->anim("manu")->set(7, pixel);
        this->anim("manu")->push();
        // this->anim("manu")->play();
      }
      else if (this->anim("manu")->hasmod("fadein"))
      {
        // this->anim("manu")->unmod();
        this->anim("manu")->mod("fadein")->period(temps)->mini(basse)->maxi(haute);
        this->anim("manu")->set(7, pixel);
        this->anim("manu")->push();
        // this->anim("manu")->play();
      }
    ...
    else if (strcmp(order->subaction, "go_m") == 0)
    {
      ...
      if (!this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod(new K32_mod_fadeout)->name("fadeout")->at(0)->at(7)->period(temps)->mini(basse)->maxi(haute);
        this->anim("manu")->set(7, pixel);
        this->anim("manu")->push();
        // this->anim("manu")->play();
      }
      else if (this->anim("manu")->hasmod("fadeout"))
      {
        // this->anim("manu")->unmod();
        this->anim("manu")->mod("fadeout")->period(temps)->mini(basse)->maxi(haute);
        this->anim("manu")->set(7, pixel);
        this->anim("manu")->push();
        // this->anim("manu")->play();
      }
    ...
    else if (strcmp(order->subaction, "temps") == 0)
    {
      int temps = order->getData(0)->toInt();
      LOGF(" temps %d", temps);
      if (!this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod(new K32_mod_fadeout)->name("fadeout")->at(0)->at(7)->period(temps);
        this->anim("manu")->push();
      }
      else if (!this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod(new K32_mod_fadein)->name("fadein")->at(0)->at(7)->period(temps);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod("fadeout")->period(temps);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod("fadein")->period(temps);
        this->anim("manu")->push();
      }
      // set_temps();
    }
    else if (strcmp(order->subaction, "haute") == 0)
    {
      int haute = order->getData(0)->toInt();
      LOGF(" haute %d", haute);
      if (!this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod(new K32_mod_fadeout)->name("fadeout")->maxi(haute);
        this->anim("manu")->push();
      }
      else if (!this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod(new K32_mod_fadein)->name("fadein")->maxi(haute);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod("fadeout")->maxi(haute);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod("fadein")->maxi(haute);
        this->anim("manu")->push();
      }
      // set_haute();
    }
    else if (strcmp(order->subaction, "basse") == 0)
    {
      int basse = order->getData(0)->toInt();
      LOGF(" basse %d", basse);
      if (!this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod(new K32_mod_fadeout)->name("fadeout")->mini(basse);
        this->anim("manu")->push();
      }
      else if (!this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod(new K32_mod_fadein)->name("fadein")->mini(basse);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadeout"))
      {
        this->anim("manu")->mod("fadeout")->mini(basse);
        this->anim("manu")->push();
      }
      else if (this->anim("manu")->hasmod("fadein"))
      {
        this->anim("manu")->mod("fadein")->mini(basse);
        this->anim("manu")->push();
      }
      // set_basse();
    }
    ...
    {
      int pixel = order->getData(0)->toInt();
      LOGF(" pixel %d", pixel);
      this->anim("manu")->set(7, pixel);
      this->anim("manu")->push();
      // this->anim("manu")->push(25, 255, 255, 255, 255, 35, 300, pixel);
      // set_niveau();
    }
  }
